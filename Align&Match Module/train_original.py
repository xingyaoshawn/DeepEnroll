...
from models.data import get_nli, get_batch, build_vocab
...
def train(args):
    ...
    #
    # word_vec = build_vocab(train_data + dev_data +
    #                    test_data)
    # logger.info('got word_vec ' + str(word_vec))
    ...
    # print("zzzzz",train_data.batch_l.size()[0])
    # ,word_emb_dim,enc_lstm_dim,pool_type,dpout_model,version
    # input_encoder = encoder(word_vecs.size(0), args.embedding_size, args.hidden_size, args.para_init)
    input_encoder = InferSent(1,args.embedding_size,int(args.hidden_size),"max",0.,1)
    ...
    for k in range(args.epoch):
        ...
        for i in range(len(train_batches)):
            ...
            # 转换为对应的encode参数
            s1_batch, s1_len = get_batch(train_src_batch,
                                     word_vecs, args.embedding_size)

            s2_batch, s2_len = get_batch(train_tgt_batch,
                                     word_vecs, args.embedding_size)

            # print(s1_batch)
            # print("s1_len , s2_len",s1_len,s2_len)
            train_src_linear = input_encoder((s1_batch, s1_len),batch_size)
            train_tgt_linear = input_encoder((s2_batch, s2_len),batch_size)
            # nli_net((s1_batch, s1_len), (s2_batch, s2_len))
...
#             para_norm ** 0.5
        ...
        if (k + 1) % args.dev_interval == 0:
            ...
            for i in range(len(dev_batches)):
                ...
                dev_batch_size = dev_src_batch.size(0)
                s1_dev_batch, s1_dev_len = get_batch(dev_src_batch,
                                         word_vecs, args.embedding_size)

                s2_dev_batch, s2_dev_len = get_batch(dev_tgt_batch,
                                         word_vecs, args.embedding_size)

                dev_src_linear = input_encoder((s1_dev_batch, s1_dev_len),dev_batch_size)
                dev_tgt_linear = input_encoder((s2_dev_batch, s2_dev_len),dev_batch_size)
    ...
    for i in range(len(test_batches)):
        ...
        test_batch_size = test_src_batch.size(0)
        s1_test_batch, s1_test_len = get_batch(test_src_batch,
                                 word_vecs, args.embedding_size)

        s2_test_batch, s2_test_len = get_batch(test_tgt_batch,
                                 word_vecs, args.embedding_size)

        test_src_linear = input_encoder((s1_test_batch, s1_test_len),test_batch_size)
        test_tgt_linear = input_encoder((s2_test_batch, s2_test_len),test_batch_size)
