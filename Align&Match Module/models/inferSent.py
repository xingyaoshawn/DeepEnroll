...
class InferSent(nn.Module):
    ...
    def __init__(self, bsize,word_emb_dim,enc_lstm_dim,pool_type,dpout_model,version):
    ...
    def forward(self, sent_tuple,batch_size):
        ...
        # 添加转换函数，将embed结果转换为attn可用格式
        emb_res = self.convert(emb,self.word_emb_dim,batch_size,self.enc_lstm_dim)
        return emb_res

    def convert(self,sent,embed_size,batch_size,hidden_size):
        sent = sent.view(-1, embed_size)
